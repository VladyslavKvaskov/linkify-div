  ...
  `;
    ...
    #linkify = (text) => {
        return text.replace(/([\w]*\:?\/\/)?([\w\d-]+\.)*[\w-]+[\.\:]\w+([\/.\?\=\&\#]?[\w-]+)*\/?/gim, (a) => {
            a = a.trim();
            this.link = a.startsWith('//') ? 'http:' + a : a;
            return `<a href="${!this.link.startsWith('http://') && !this.link.startsWith('https://') ? `http://${this.link}` : this.link}" target="_blank" class="url-from-txt">${a}</a>`;
        });
    };
